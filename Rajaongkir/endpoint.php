    ...
    public function getFullName($API_NAME)
    {
        ...
        switch ($API_NAME) {
            ...
            case 'list_provinsi':
                ...
                $FULL_URL = $url . "/starter/province"; // sesuai dengan url yg diberikan rajaongkir

                break;

            case 'list_city':

                $FULL_URL = $url . "/starter/city"; // sesuai dengan url yg diberikan rajaongkir

                break;

            case 'list_cost':

                $origin=$_POST['origin']; // 501
                $destination=$_POST['destination']; // 114
                $weight=$_POST['weight']; // 1700
                $courier=$_POST['courier']; // jne


                $FULL_URL = $url . "/starter/cost?"; // sesuai dengan url yg diberikan rajaongkir
                $FULL_URL .= "origin=$origin";
                $FULL_URL .= "&destination=$destination";
                $FULL_URL .= "&weight=$weight";
                $FULL_URL .= "&courier=$courier";
